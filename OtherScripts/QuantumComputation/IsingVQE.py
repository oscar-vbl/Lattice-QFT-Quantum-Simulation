...
def getVQEAnsatzConfig(numQubits, theta_angles, numAncilla=0):
    ...
    configuration["Gates"] += [
        {"gate": "RZ", "qubit": {"Number": 0}, "angle": theta_angles[0]}
    ]
    configuration["Gates"] += [
        {"gate": "RY", "qubit": {"Number": 0}, "angle": theta_angles[1]}
    ]
        ...
        configuration["Gates"] += [
            {
                "gate": "RY",
                "qubit": {"Number": qubit},
                "angle": theta_angles[used_theta + qubit],
            },
        ]
...
class VQESolver:
    ...
    def calculateEnergyValue(self, ansatz_circuit, shotsNum=1024):
        ...
        for pauli_string, qubits, coeff in self.hamiltonian.to_sparse_list():
            ...
            for qubit in qubits:
                ...
                if pauli == "X":
                    hadamard_gate = {"gate": "H", "qubit": {"Number": qubit}}
    ...
    def minimizeEnergy(self, initial_angles, shotsNum=1024):
        ...
        result = minimize(
            self.getEnergy, initial_angles, args=(shotsNum), method="COBYLA"
        )
        return result
...
def VQE_Energy_Field(
    ...
    for h_field in h_values:
        ...
        if validateRuns:
            ...
            best_energy_validated = float(
                validator.getEnergy(best_angles, shotsNum=shots_validate)
            )
    ...
    if plot:
        if False:
            ...
            plt.title(
                f"VQE Ground State Energy for Transverse Ising Model ({numQubits} qubits, {shotsNum} shots)"
            )
            plt.grid()
            plt.legend()
            plt.show()
...
if __name__ == "__main__":
    ...
    if True:
        energy_values = VQE_Energy_Field(
            numQubits,
            J,
            h_values,
            shotsNum=shots,
            ...
            plot=True,
            compareNumerical=compareNumerical,
        )
