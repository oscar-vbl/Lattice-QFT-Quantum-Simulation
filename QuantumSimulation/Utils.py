...
def load_data(folderName, fileName, rootPath=None, indexSet=None):
    ...
    if fileName.endswith(".csv"):
        ...
        if indexSet:
            data.set_index(indexSet, inplace=True)
...
def func_return(
    func: Callable, params: Mapping = {}, default=None, expect_type: type | None = None
) -> Any:
    ...
    try:
        ...
        if expect_type is not None:
            assert isinstance(result, expect_type), (
                f"WARNING: Function {func.__name__} did not return expected type {expect_type}, got {type(result)}. Review parameters..."
            )
        return result
