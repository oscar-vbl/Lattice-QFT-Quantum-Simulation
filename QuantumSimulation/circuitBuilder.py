...
def buildCircuit(configuration):
    ...
    ----------
    configuration: dict
        Configuration dictionary specifying the circuit to be built. It should include the following
    ...
    -------
    QuantumCircuit
        The quantum circuit built according to the configuration.
    """
    ...
    if measureQubits:
        circuit = QuantumCircuit(
            QuantumRegister(numQubits), ClassicalRegister(measureQubits)
        )
    else:
        circuit = QuantumCircuit(QuantumRegister(numQubits))
    if ancillaQubits > 0:
...
def addGate(circuit: QuantumCircuit, gate: dict, ancilla=None):
    """
    ...
    gate: dict
        Configuration of the gate to be added. Must include "gate" key specifying the type of gate and other keys depending on the gate type.
    ancilla: AncillaRegister, optional
        The ancilla register if needed for the gate.
    """
    gateType = gate["gate"]
    if not ancilla:
        ancilla = circuit.ancillas
    if gateType == "CNOT":
        # Control qubit of CNOT gate
        controlQubitNum = gate["control"]["Number"]
        isAncilla = gate["control"].get("Ancilla", False)
        if isAncilla:
            controlQubit = ancilla[controlQubitNum]
        else:
            controlQubit = controlQubitNum
        # Target qubit of CNOT gate
        targetQubitNum = gate["target"]["Number"]
        isAncilla = gate["target"].get("Ancilla", False)
        if isAncilla:
            targetQubit = ancilla[targetQubitNum]
        else:
            targetQubit = targetQubitNum
        # Add CNOT gate
