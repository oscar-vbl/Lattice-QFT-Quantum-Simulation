...
def get_ansatz_comparison(config,
                        qubits_num_values,
                        runs=5,
                        parallel_runs=None,
                        top_runs=5,
    ...
    for qubits_num in qubits_num_values:
        ...
        simulator_save_temp = f"Simulator_{ansatz_type}_{qubits_num}Qubits" + "_Run{value}.pkl"
        if use_simulated_data:
            # Load simulator data if available
            runs_simulations = load_simulator_instance(analysis_name, list(range(total_runs)), simulator_save_temp)
            sims_are_run = all([runs_simulations[run]["simulator"] is not None for run in runs_simulations])
        else:
            sims_are_run = False

        if not sims_are_run:
            print(f"{getTimer()} INFO: Ansatz {ansatz_type} for L={qubits_num}. Starting {total_runs} parallel runs...")
            ...
            with concurrent.futures.ProcessPoolExecutor(max_workers=parallel_runs) as executor:
                ...
                futures = [executor.submit(quiet_get_simulation_data, qubits_num_config) for _ in range(total_runs)]
                ...
                for future in concurrent.futures.as_completed(futures):
                    try:
                        ...
                        print(f"{getTimer()} INFO: Run completed for {qubits_num} qubits in {duration:.2f} s, with energy: {energy:.4f}")
                        
                    except Exception as exc:
                        print(f"{getTimer()} WARNING: Exception raised by run for {qubits_num} qubits: {exc}")
            ...
            if save_simulated_data:
                for run in range(len(sorted_results)):
                    save_data(sorted_results[run]["simulator"], analysis_name, simulator_save_temp.format(value=run), rootPath=data_folder)
        ...
        for run in range(top_k):
            ...
            simulator, duration = best_runs[run]["simulator"], best_runs[run]["duration"]
            ...
            comparisons[qubits_num]["Overlap_Values"].append(np.abs(np.vdot(num_gs, initial_state))**2)
            comparisons[qubits_num]["Sim_Energy_Values"].append(vacuum_energy)
            comparisons[qubits_num]["Dev_Energy_Values"].append((vacuum_energy - num_energy) / np.abs(num_energy))
            comparisons[qubits_num]["Time_Values"].append(vqe_duration)
            comparisons[qubits_num]["Optimization_History"].append(simulator.optimization_history)

        print(f"{getTimer()} INFO: Overlap for {qubits_num} qubits:", np.mean(comparisons[qubits_num]["Overlap_Values"]))
        print(f"{getTimer()} INFO: Energy error for {qubits_num} qubits:", np.mean(comparisons[qubits_num]["Dev_Energy_Values"]))

        comparisons[qubits_num]["Overlap_Mean"]  = np.mean(comparisons[qubits_num]["Overlap_Values"])
        comparisons[qubits_num]["Overlap_Error"] = np.std(comparisons[qubits_num]["Overlap_Values"])

        comparisons[qubits_num]["Sim_Energy_Mean"]  = np.mean(comparisons[qubits_num]["Sim_Energy_Values"])
        comparisons[qubits_num]["Sim_Energy_Error"] = np.std(comparisons[qubits_num]["Sim_Energy_Values"])

        comparisons[qubits_num]["Dev_Energy_Mean"]  = np.mean(comparisons[qubits_num]["Dev_Energy_Values"])
        comparisons[qubits_num]["Dev_Energy_Error"] = np.std(comparisons[qubits_num]["Dev_Energy_Values"])

        comparisons[qubits_num]["Time_Mean"]  = np.mean(comparisons[qubits_num]["Time_Values"])
        comparisons[qubits_num]["Time_Error"] = np.std(comparisons[qubits_num]["Time_Values"])
...
if __name__ == "__main__":
    ...
    if True:
        ...
        for ansatz in ansatzes:
            ...
            for reps in reps_num:
                ...
                comparisons = get_ansatz_comparison(
                    reps_config, [qubits_num],
                    analysis_name = analysis_name_reps
                )
                if reps != qubits_num:
        ...
        fig_err, axes = plot_validation_ansatzes( reps_num, ansatzes_comparison, x_label="Ansatz Repetitions")
        fig_dur, ax    = plot_duration_ansatzes(   reps_num, ansatzes_comparison, x_label="Ansatz Repetitions")
        # Save plot
        file_name = "00_02_Overlap_Vs_Reps"
        save_data(fig_err,           analysis_name, f"{file_name}_Errors.png",   rootPath=plt_folder, dpi=300)
        save_data(fig_err,           analysis_name, f"{file_name}_Errors.pdf",   rootPath=plt_folder, dpi=300)
        save_data(fig_dur,           analysis_name, f"{file_name}_Duration.png", rootPath=plt_folder, dpi=300)
        save_data(fig_dur,           analysis_name, f"{file_name}_Duration.pdf", rootPath=plt_folder, dpi=300)
    ...
    if True:
        ...
        for ansatz in ansatzes:
            ...
            for L in L_vals:
                ...
                transpiled_circ = transpile(ansatz_circuit, basis_gates=ibm_basis_gates, optimization_level=2)
        ...
        save_data({"depths": depths, "cnots": cnots}, analysis_name, f"{file_name}.pkl", rootPath=plt_folder)
    ...
    if True:
        ...
        for ansatz in ansatzes:
            ...
            for L in L_vals:
                ...
                ansatz_var = calculate_global_gradient_variance(ansatz_circuit, H_sparse, num_samples=100)
