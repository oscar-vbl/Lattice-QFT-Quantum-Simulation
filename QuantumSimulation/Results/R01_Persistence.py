...
def fit_persistence_e0(config, e0_values, e0_data=None, initial_state=None):
    ...
    for e0 in e0_values:
        if e0_data is not None:
            try:
                ...
                print(
                    f"{getTimer()} WARNING: Data for e0={e0} not found, running simulation."
                )
                ...
                print(
                    f"{getTimer()} INFO: Simulation for {qubits_num} qubits and e0={e0} took {(duration):.2f} seconds."
                )
        else:
            simulator, duration = get_simulation_data(
                config, initial_state=initial_state
...
def fit_persistence_qubits_num(config, qubits_num_values, qubits_num_data=None):
    ...
    for qubits_num in qubits_num_values:
        if qubits_num_data is not None:
            try:
                ...
                print(
                    f"{getTimer()} WARNING: Data for L={qubits_num} not found, running simulation."
                )
...
if __name__ == "__main__":
    ...
    if True:  # 1. Get evolution data for different values of L
        ...
        fit_results_df = fit_persistence_qubits_num(
            config, qubit_num_values, qubits_num_data
        )
        ...
        save_data(
            fit_results_df, analysis_name, f"{file_name}.csv", rootPath=plt_folder
        )
    ...
    if True:  # 3. Plot persistence and regimes for best fit
        ...
        plot_params = parseDictToPlot(
            {
                ...
                **qubits_num_config["Temporal Evolution"]["Quench"][
        ...
        fig, ax = plot_persistenece_vs_time_regimes(
            evolution_data, cut_off_times, plot_params
        )
        # Save plot and data
        save_data(fig, analysis_name, f"{file_name}.png", rootPath=plt_folder, dpi=300)
        save_data(
    ...
    if False:  # 4. Show best fit for different values of e0 (Strong field regime)
        ...
        step2 = 0.1
        e0_values = np.concatenate(
            (e0_values, np.arange(min_e0_2, max_e0_2 + step2, step2))
        )
        ...
        e0_data = load_evolution_and_initial(
            analysis_name,
            e0_values,
        ...
        fig, ax = plot_gamma_vs_e0(
            fit_results_df, params=plot_params, title_suffix=" (Weak Field Regime)"
        )
        # Save plot and data
        save_data(fig, analysis_name, f"{file_name}.png", rootPath=plt_folder, dpi=300)
        save_data(
    ...
    if True:  # 5. Plot log Gamma vs 1/e0 in logartihmic scale for Schwinger regime
        ...
        e0_values_schwinger = fit_results_df[
        ...
        field_values = (
            fit_reg_results_df["E_Physical"].apply(lambda x: np.mean(x)).values
        )
        ...
        fig, axes, fit_params = plot_gamma_vs_electricField(
            gamma_simulated, e0_values, field_values, plot_params
        )
        # Save plot and data
        save_data(fig, analysis_name, f"{file_name}.png", rootPath=plt_folder, dpi=300)
        save_data(
